...
static void plan_row_deleted (GtkTreeModel       *model,
  ...
  if(delete_i<hg->plan_insert_i){
    for(i_plan=delete_i;i_plan<hg->plan_insert_i;i_plan++){
      swap_plan(&hg->plan[i_plan],&hg->plan[i_plan+1]);
    }
  }
  else if(delete_i>hg->plan_insert_i){
    hg->plan_insert_i++;
    delete_i--;
    for(i_plan=delete_i;i_plan>hg->plan_insert_i;i_plan--){
      swap_plan(&hg->plan[i_plan],&hg->plan[i_plan-1]);
...
void create_plan_dialog(typHOE *hg)
{
  ...
  // Object
  {
	...
	my_signal_connect (hg->plan_obj_combo,
      ...
      my_signal_connect (hg->plan_dexp_adj, "value_changed",
			 cc_get_adj_double,
			 &hg->plan_obj_dexp);
      spinner =  gtk_spin_button_new (hg->plan_dexp_adj, 1, 1);
      gtk_spin_button_set_wrap (GTK_SPIN_BUTTON (spinner),
				FALSE);
	...
	my_signal_connect (hg->plan_obj_combo,
      ...
      my_signal_connect (hg->plan_nw_check, "toggled",
			 cc_get_toggle, &hg->plan_nw);
  ...
  case INST_IRD:
	...
	for(i_mode=0;i_mode<NUM_IRD_COMP_MODE;i_mode++){
	  gtk_list_store_append(store, &iter);
	  gtk_list_store_set(store, &iter, 0, ird_comp_mode_name[i_mode],
			     1, i_mode, -1);
	}
  ...
  case INST_IRD:
...
#ifdef USE_GTK3      
    button=gtkut_button_new_from_icon_name("Dark","list-add");
#else
    button=gtkut_button_new_from_stock("Dark",GTK_STOCK_ADD);
#endif
    gtk_box_pack_start(GTK_BOX(hbox),button,FALSE,FALSE,0);
    my_signal_connect(button,"pressed",
		      add_Dark, 
    ...
    hg->plan_dark_dexp=60;
    adj = (GtkAdjustment *)gtk_adjustment_new(hg->plan_dark_dexp,
					      1.0, 1000, 1.0, 10.0, 0);
    my_signal_connect (adj, "value_changed",
		       cc_get_adj_double,
		       &hg->plan_dark_dexp);
    ...
    hg->plan_dark_repeat=10;
    adj = (GtkAdjustment *)gtk_adjustment_new(hg->plan_dark_repeat,
					      1, 50, 1.0, 1.0, 0);
    my_signal_connect (adj, "value_changed",
		       cc_get_adj,
		       &hg->plan_dark_repeat);
    ...
    hg->plan_dark_daytime=FALSE;
    check = gtk_check_button_new_with_label("Daytime");
    gtk_box_pack_start(GTK_BOX(hbox),check,FALSE, FALSE, 0);
    my_signal_connect (check, "toggled",
		       cc_get_toggle,
		       &hg->plan_dark_daytime);
  ...
  case INST_KOOLS:
    label = gtk_label_new ("   KOOLS-IFU Setup : Grism ");
    gtk_box_pack_start(GTK_BOX(hbox),label,FALSE,FALSE,0);
    
    {
      ...
      for(i_use=0;i_use<NUM_KOOLS_GRISM;i_use++){
	gtk_list_store_append(store, &iter);
	gtk_list_store_set(store, &iter, 
			   0, kools_grism_name[i_use],
			   1, i_use, 
			   2, &color_black,
			   3, &col_kools_grism[i_use],
			   -1);
      }
...
static void cc_obj_list (GtkWidget *widget, gpointer gdata)
{
  ...
  case INST_IRD:
    if(hg->e_list!=-1){
      hg->plan_obj_dexp=hg->obj[hg->e_list].dexp;
  ...
  case INST_KOOLS:
    if(hg->e_list!=-1){
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(hg->plan_ag_check),
				   hg->obj[hg->e_list].kools.ag);
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(hg->plan_nw_check),
				   hg->obj[hg->e_list].kools.nw);
    }    
    break;
  }
}
...
GtkWidget *make_plan_menu(typHOE *hg){
  ...
  switch(hg->inst){
  case INST_KOOLS:
    tmp=g_strdup("Create Shell-Script Files");
    break;

  default:
    tmp=g_strdup("Write Plan OPE");
    break;
  }
#ifdef USE_GTK3
  image=gtk_image_new_from_icon_name ("document-save", GTK_ICON_SIZE_MENU);
  popup_button =gtkut_image_menu_item_new_with_label (image, tmp);
#else
  image=gtk_image_new_from_stock (GTK_STOCK_SAVE, GTK_ICON_SIZE_MENU);
  popup_button =gtk_image_menu_item_new_with_label (tmp);
	     ...
	     GtkTreeModel *items_model)
{
  ...
  column=gtk_tree_view_column_new_with_attributes ("Moon",
						   renderer,
						   "text",  COLUMN_PLAN_MOON,
#ifdef USE_GTK3
						   "background-rgba", COLUMN_PLAN_MOON_COLBG,
#else
						   "background-gdk", COLUMN_PLAN_MOON_COLBG,
#endif
						   NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer,
					  plan_cell_data_func,
...
void plan_hsc_cell_data_func(GtkTreeViewColumn *col , 
			     GtkCellRenderer *renderer,
			     GtkTreeModel *model, 
			     GtkTreeIter *iter,
			     gpointer user_data)
...
void plan_kools_cell_data_func(GtkTreeViewColumn *col , 
			       GtkCellRenderer *renderer,
			       GtkTreeModel *model, 
			       GtkTreeIter *iter,
			       gpointer user_data)
{
  ...
  case COLUMN_PLAN_FIL:
    if(int_value>=0){
      str=g_strdup_printf("%s",kools_grism_name[int_value]);
    }
    else{
      str=NULL;
    }
...
gchar * ird_make_plan_txt(typHOE *hg, PLANpara plan){
  gchar *bu_tmp=NULL, *pa_tmp=NULL, *sv1_tmp=NULL, *sv2_tmp=NULL,
    *set_tmp=NULL, *aomode_tmp=NULL, *is_tmp=NULL, *bin_tmp=NULL, *col_tmp=NULL,
    *dith_tmp=NULL, *exp_tmp=NULL, *ndr_tmp=NULL, *adi_tmp=NULL, *ret_txt=NULL;
  gint exp10;
  ...
  case PLAN_TYPE_OBJ:
    ...
    if(plan.dexp<10){
      exp_tmp=g_strdup_printf("%.1lf",plan.dexp);
    }
    else{
      exp_tmp=g_strdup_printf("%.0lf",plan.dexp);
    }
    ...
    ret_txt=g_strdup_printf("%s\"%s\"%s%s %ssec x%d",
			    bu_tmp,
			    hg->obj[plan.obj_i].name,
			    aomode_tmp,
			    adi_tmp,
  ...
  case PLAN_TYPE_FLAT:
    ret_txt=g_strdup_printf("Flat : %s",
			    ird_flat_mode_name[plan.cal_mode]);
    break;

  case PLAN_TYPE_COMP:
    ret_txt=g_strdup_printf("Comparison : %s",
			    ird_comp_mode_name[plan.cal_mode]);
    break;

  case PLAN_TYPE_DARK:
    exp10=(gint)(plan.dexp*10.0);
    if(exp10%10==0){
      ret_txt=g_strdup_printf("Dark : %.0lfsec x%d",
			      plan.dexp, plan.repeat);
    }
    else{
      ret_txt=g_strdup_printf("Dark : %.1lfsec x%d",
			      plan.dexp, plan.repeat);
    }
    break;
  ...
  case PLAN_TYPE_SETUP:
  case PLAN_TYPE_I2:
    ret_txt=g_strdup_printf("### (Remove this line for IRD)");
    break;
...
gchar * kools_make_plan_txt(typHOE *hg, PLANpara plan){
  ...
  case PLAN_TYPE_OBJ:
    ...
    ret_txt=g_strdup_printf("%s\"%s\" %dsec x%d  %s",
			    bu_tmp,
			    hg->obj[plan.obj_i].name,
			    plan.exp,
			    plan.repeat,
...
static void
add_1Object_IRD (typHOE *hg, gint i, gint obj_i, gdouble dexp, gint repeat)
{
  gint i_plan;
  ...
  hg->plan[i].time=ird_obj_time(hg->plan[i],
				hg->oh_acq,
				ird_oh_ao(hg,
					  hg->plan[i].aomode,
					  hg->plan[i].obj_i));
...
static void
add_1Object_KOOLS (typHOE *hg, gint i, gint obj_i, gint exp, gint repeat)
{
  gint i_plan;
  ...
  hg->plan[i].time=kools_obj_time(hg->plan[i],
				  hg->oh_acq,
				  hg->oh_ngs1,
				  hg->oh_ngs2);
...
add_Object (GtkWidget *button, gpointer data)
{
  ...
  if(hg->plan_obj_i==-1){
	...
	break;
      ...
      case INST_IRD:
	add_1Object_IRD(hg, i+i_list, i_list,
			hg->obj[i_list].dexp, 
			hg->obj[i_list].repeat);
	break;

      case INST_KOOLS:
	add_1Object_KOOLS(hg, i+i_list, i_list, 
			  hg->obj[i_list].exp, 
			  hg->obj[i_list].repeat);
	break;
      }
    ...
    case INST_IRD:
      add_1Object_IRD(hg, i, hg->plan_obj_i,
		      hg->plan_obj_dexp, hg->plan_obj_repeat);
      break;
...
add_Comp (GtkWidget *button, gpointer data)
{
  ...
  case INST_IRD:
    hg->plan[i].cal_mode=hg->plan_comp_mode;
    hg->plan[i].repeat=ird_comp_repeat[hg->plan[i].cal_mode];
    break;
  }
...
add_Flat (GtkWidget *button, gpointer data)
{
  ...
  case INST_IRD:
    hg->plan[i].cal_mode=hg->plan_flat_mode;
    hg->plan[i].repeat=ird_flat_repeat[hg->plan[i].cal_mode];
    break;
}
...
static void
add_Dark (GtkWidget *button, gpointer data)
{
  GtkTreeIter iter;
  typHOE *hg = (typHOE *)data;
  GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(hg->plan_tree));
  ...
  hg->plan[i].type=PLAN_TYPE_DARK;

  switch(hg->inst){
  case INST_IRD:
    hg->plan[i].dexp=hg->plan_dark_dexp;
    hg->plan[i].repeat=hg->plan_dark_repeat;
    break;
  }

  hg->plan[i].daytime=hg->plan_dark_daytime;
  hg->plan[i].time=dark_time(hg->plan[i], hg);
			   ...
			   gint i_plan)
{
  ...
  gtk_list_store_set (GTK_LIST_STORE(model), &iter,
		      ...
		      -1);
  if(hg->plan_moon_flag){
    gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			COLUMN_PLAN_MOON_COLBG,&color_gray2,
			-1);
  }
  else{
    if(hg->plan[i_plan].moon.sep <= 0){
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_MOON_COLBG,NULL,
			  -1);
    }
    else if(hg->plan[i_plan].moon.sep < 15){
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_MOON_COLBG,&color_lred,
			  -1);
    }
    else if(hg->plan[i_plan].moon.sep < 45){
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_MOON_COLBG,&color_lorange,
			  -1);
    }
    else if(hg->plan[i_plan].moon.sep < 90){
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_MOON_COLBG,&color_lgreen,
			  -1);
    }
    else{
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
  ...
  if(hg->plan[i_plan].setup>=0){
    switch(hg->inst){
    case INST_IRCS:
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_COLBG,
			  &col_ircs_setup[hg->plan[i_plan].setup],
			  -1);
      break;
    case INST_HSC:
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_COLBG,
			  &col_hsc_setup[hg->plan[i_plan].setup],
			  -1);
      break;
    ...
    case INST_KOOLS:
      gtk_list_store_set (GTK_LIST_STORE(model), &iter,
			  COLUMN_PLAN_COLBG,
			  &col_kools_grism[hg->plan[i_plan].setup],
			  -1);
      break;
...
void ird_init_plan(typHOE *hg)
{
  ...
  // Comp 1
  {
    ...
    hg->plan[i_plan].type=PLAN_TYPE_COMP;
    hg->plan[i_plan].cal_mode=IRD_COMP_STAR_COMB;
    hg->plan[i_plan].daytime=TRUE;
    hg->plan[i_plan].time=comp_time(hg->plan[i_plan], hg);
    hg->plan[i_plan].txt=make_plan_txt(hg,hg->plan[i_plan]);
  }
  ...
  // Comp 2
  {
    ...
    hg->plan[i_plan].type=PLAN_TYPE_COMP;
    hg->plan[i_plan].cal_mode=IRD_COMP_STAR_THAR;
    hg->plan[i_plan].daytime=TRUE;
    hg->plan[i_plan].time=comp_time(hg->plan[i_plan], hg);
    hg->plan[i_plan].txt=make_plan_txt(hg,hg->plan[i_plan]);
  }
  ...
  // Comp 3
  {
    ...
    hg->plan[i_plan].type=PLAN_TYPE_COMP;
    hg->plan[i_plan].cal_mode=IRD_COMP_COMB_THAR;
    hg->plan[i_plan].daytime=TRUE;
    hg->plan[i_plan].time=comp_time(hg->plan[i_plan], hg);
    hg->plan[i_plan].txt=make_plan_txt(hg,hg->plan[i_plan]);
  }
...
void kools_init_plan(typHOE *hg)
{
  gint i_plan=0, i_mode;
...
glong remake_tod(typHOE *hg, GtkTreeModel *model, gboolean moon_flag) 
{
  ...
  if(moon_flag){
    //printf("#### Remaking ToD   TRUE  #####\n");
    hg->plan_moon_flag=TRUE;
  }
  else{
    //printf("#### Remaking ToD   FALSE  #####\n");
    hg->plan_moon_flag=FALSE;
  }
  ...
  for(i_plan=0;i_plan<hg->i_plan_max;i_plan++){
    ...
    if(hg->plan[i_plan].type==PLAN_TYPE_OBJ){
      ...
      case INST_HDS:
      case INST_IRD:
	total_exp+=(gint)(hg->plan[i_plan].dexp*(gdouble)hg->plan[i_plan].repeat);
	break;
...
void remake_sod(typHOE *hg) 
{
  ...
  for(i_plan=0;i_plan<hg->i_plan_max;i_plan++){
	...
	break;
      ...
      case INST_IRD:
	total_exp+=(gint)(hg->plan[i_plan].exp*(gdouble)hg->plan[i_plan].repeat);
	break;
      }
    }
...
focus_plan_item (GtkWidget *widget, gpointer data)
{
  ...
  /*
  printf("%d : %s   Az0=%+.1lf El0=%.1lf   Az1=%+.1lf El1=%.1lf  MoonSep=%.1lf\n",
	 hg->plot_i_plan,
	 ...
	 hg->plan[hg->plot_i_plan].moon.sep);
  */
...
static void view_onRowActivated (GtkTreeView        *treeview,
  ...
  if (gtk_tree_model_get_iter(model, &iter, path)){
	...
	break;
      case PLAN_TYPE_OBJ:
	ird_do_edit_obj(hg,model,iter,i_plan);
	break;
      case PLAN_TYPE_DARK:
...
static void do_edit_dark (typHOE *hg, 
			  GtkTreeModel *model, 
			  GtkTreeIter iter, 
			  gint i_plan)
{
  ...
  dialog = gtk_dialog_new_with_buttons("HOE : Plan Edit (Dark)",
				       GTK_WINDOW(hg->plan_main),
				       GTK_DIALOG_MODAL,
#ifdef USE_GTK3
				       "_Cancel",GTK_RESPONSE_CANCEL,
  ...
  sprintf(tmp,"[Plan #%d]  Dark :", i_plan);
  label = gtk_label_new (tmp);
#ifdef USE_GTK3
  gtk_widget_set_halign (label, GTK_ALIGN_START);
  gtk_widget_set_valign (label, GTK_ALIGN_CENTER);
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
    ...
    switch(hg->inst){
    case INST_IRD:
      tmp_plan.time=dark_time(tmp_plan, hg);
			   ...
			   gint i_plan)
{
  ...
  
  {
    ...
    case INST_TRICCS:
      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, 0, "Shack-Hartmann Test",
			 1, PLAN_FOCUS2, -1);
      if(hg->plan[i_plan].focus_mode==PLAN_FOCUS2) iter_set=iter;
      break;
    }
...
static void ird_do_edit_obj (typHOE *hg, 
			     GtkTreeModel *model, 
			     GtkTreeIter iter, 
			     gint i_plan)
{
  ...
  my_signal_connect (hg->plan_e_dexp_adj, "value_changed",
		     cc_get_adj_double,
		     &hg->plan_obj_dexp);
  spinner =  gtk_spin_button_new (hg->plan_e_dexp_adj, 1, 1);
  gtk_spin_button_set_wrap (GTK_SPIN_BUTTON (spinner),
			    FALSE);
  my_entry_set_width_chars(GTK_ENTRY(&GTK_SPIN_BUTTON(spinner)->entry),6);
  gtk_box_pack_start(GTK_BOX(hbox),spinner,FALSE,FALSE,0);
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
    ...
    tmp_plan.time=ird_obj_time(tmp_plan,
			       hg->oh_acq,
			       ird_oh_ao(hg,
					  tmp_plan.aomode,
					  tmp_plan.obj_i));
...
static void kools_do_edit_obj (typHOE *hg, 
			       GtkTreeModel *model, 
			       GtkTreeIter iter, 
			       gint i_plan)
{
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
    ...
    tmp_plan.time=kools_obj_time(tmp_plan,
				 hg->oh_acq,
				 hg->oh_ngs1,
				 hg->oh_ngs2);
...
gint ird_obj_time(PLANpara plan, gint oh_acq, gint oh_ao){
  gint ret_time;

  ret_time
    =(gint)(oh_acq+oh_ao
	    +(plan.dexp+IRD_TIME_READOUT)*(gdouble)plan.repeat);

  return(ret_time);
}


gint kools_obj_time(PLANpara plan, gint oh_acq, gint oh_pc, gint oh_ag){
  gint ret_time=0;
...
gint comp_time(PLANpara plan, typHOE *hg){
  ...
  case INST_IRCS:
    ...
    break;
  case INST_IRD:
    ret=(gint)((ird_comp_exp[plan.cal_mode]+(gdouble)IRD_TIME_READOUT)
	       *(gdouble)ird_comp_repeat[plan.cal_mode]);
    break;
  }
...
gint flat_time(PLANpara plan, typHOE *hg){
  ...
  case INST_IRD:
    ret=(gint)((ird_flat_exp[plan.cal_mode]+(gdouble)IRD_TIME_READOUT)
	       *(gdouble)ird_flat_repeat[plan.cal_mode]);
    break;
  }
...
gint dark_time(PLANpara plan, typHOE *hg){
  gint ret=0;
  gint i_set;
  gint rep;
  gint coadds;
  ...
  switch(hg->inst){
  case INST_IRD:
    ret=(gint)((plan.dexp + (gdouble)IRD_TIME_READOUT)*(gdouble)plan.repeat);
    break;
  }
